...
func execIncr(db *DB, args [][]byte) resp.Reply {
	...
	if bytes != nil {
		...
		return reply.NewIntReply(val + 1)
	}
	db.PutEntity(key, &database.DataEntity{
		Data: []byte("1"),
	})
...
func execIncrBy(db *DB, args [][]byte) resp.Reply {
	...
	if bytes != nil {
		...
		return reply.NewIntReply(val + delta)
	}
	db.PutEntity(key, &database.DataEntity{
		Data: args[1],
	})
	db.addAof(utils.ToCmdLine2("incrby", args...))
	return reply.NewIntReply(delta)
}
...
func execSetRange(db *DB, args [][]byte) resp.Reply {
	...
	if errNative != nil {
		return reply.NewErrReply(errNative.Error())
	}
	value := args[2]
	bytes, err := db.getAsString(key)
	if err != nil {
...
func execGetRange(db *DB, args [][]byte) resp.Reply {
	...
	return reply.NewBulkReply(bytes[startIdx:endIdx])
}
